...
import { CheckCircle2, ChevronLeft, MapPin, Navigation2, Phone } from 'lucide-react';
...
export default function AgentNavigation() {
  ...
  useEffect(() => {
    ...
    const resolveDestination = async () => {
      try {
        ...
        const restaurant = await restaurantService.getRestaurantById(assignment.restaurantId);
  ...
  }, [assignment, isHeadingToCustomer]);
  ...
  useEffect(() => {
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      ...
      () => {
        // Keep last known position if geolocation fails.
      },
      {
  ...
  const destinationLabel = isHeadingToCustomer
    ...
    : `Restaurant #${assignment.restaurantId}`;
    ...
    <div className="absolute inset-0 flex flex-col overflow-hidden bg-slate-50">
      ...
      <div className="absolute inset-0 z-0">
        <MapContainer center={currentPosition} zoom={15} scrollWheelZoom={false} className="h-full w-full">
          ...
          {route.length > 1 ? (
            <Polyline positions={route} color="#6366f1" weight={6} opacity={0.95} />
          ) : null}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-black uppercase tracking-widest text-indigo-600">
              {isHeadingToCustomer ? 'Heading to customer' : 'Heading to pickup'}
            </p>
      ...
      <div className="relative z-10 rounded-t-[2.2rem] border-t border-slate-200 bg-white px-6 pb-9 pt-5 shadow-[0_-20px_50px_rgba(0,0,0,0.12)]">
        ...
        <div className="mb-4 flex items-start justify-between">
          ...
          <a
            ...
            className="ml-4 flex h-12 w-12 items-center justify-center rounded-full border border-indigo-200 bg-indigo-50 text-indigo-600"
          >
            <Phone size={18} />
          </a>
        </div>
        ...
        {assignment.status === 'READY' || assignment.status === 'CONFIRMED' ? (
          <button
            onClick={() => void handleStatusUpdate('PICKED_UP')}
          ...
          >
            {actionLoading ? 'Updating...' : <><CheckCircle2 size={18} /> Confirm Pickup</>}
          </button>
          ...
          >
            {actionLoading ? 'Completing...' : <><CheckCircle2 size={18} /> Mark Delivered</>}
          </button>
        ) : (
